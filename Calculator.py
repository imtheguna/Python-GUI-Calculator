...
def clickbut(number):  
     global operator
     l=len(operator)
     if(l>1 and ( number in ['+','/','-','*'])):
          if(operator[l-1] in ['+','/','-','*'] ):
               operator=operator[:l-1]
     if(l==0 and ( number in ['+','/','-','*'])):
               operator="0"+str(number)
     else:
...
metext=Entry(me,font=("Courier New",12,'bold'),textvar=textin,width=31,bd=5,bg='powder blue')
metext.place(x=15,y=50)
...
icon=tkinter.PhotoImage(file="E:\icons8-clear-symbol-60.png")

butclear=Button(me,padx=14,pady=83,bd=4,bg='white',text="CE",command=clrbut,font=("Courier New",16,'bold'))
butclear.place(x=270,y=170)

butclear=Button(me,padx=15,pady=14,bd=4,bg='white',image=icon,command=clrbut1,font=("Courier New",16,'bold'))
